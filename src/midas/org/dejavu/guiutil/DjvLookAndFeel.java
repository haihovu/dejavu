...
public class DjvLookAndFeel
{
	...
	private static LookAndFeel loadLookAndFeel(String className)
	{
		...
		try
		{
			lafClass = DjvLookAndFeel.class.getClassLoader().loadClass(className);
		...
		{
			DjvSystem.logInfo(Category.DESIGN, className + " is not a valid look and feel class");
		}
		catch (NoSuchMethodException e)
		{
			DjvSystem.logInfo(Category.DESIGN, "Could not locate default constructor of " + lafClass);
		}
		catch (ClassNotFoundException e)
		{
			DjvSystem.logInfo(Category.DESIGN, "Could not locate LAF " + className);
		}
		catch(Exception e)
		{
			DjvSystem.logInfo(Category.DESIGN, DjvExceptionUtil.simpleTrace(e));
		}
		...
		class LafUpdater implements Runnable
		{
			...
			public void run()
			{
				...
				synchronized(s_StaticLock)
				{
					...
					if(lafName == null)
					{
						DjvSystem.logError(Category.DESIGN, "Unable to locate the default system look and feel");
						return;
					}
					LookAndFeel lafObj = s_LafMap.get(lafName);
					if(lafObj == null)
					{
						DjvSystem.logWarning(Category.DESIGN, "Failed to instantiate look and feel "
