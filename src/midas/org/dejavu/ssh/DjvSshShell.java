...
public class DjvSshShell
{
	 ...
	 * Input reader task
	 */
	private DjvBackgroundTask m_StdInReader;
	...
	private class MyChannelListener implements ChannelEventListener
	{
		...
		{
			synchronized(DjvSshShell.this)
			{
				DjvSshShell.this.notifyAll();
			}
		}
	 ...
	 */
	private class BgTaskInputReader extends DjvBackgroundTask
	{
	 ...
	 */
	public DjvSshShell(SshClient sshClient, String encoding) throws IOException
	{
	...
	public String discoverCmdLinePrompt(DjvSshAuthInfo authInfo) throws IOException, InterruptedException
	{
		String line;
	...
	{
		DjvSshClient client = new DjvSshClient();
		...
		try {
			...
			try {
				DjvSshShell sh = client.openShell("UTF-8");
				try {
					sh.start(1000);
					sh.type("ps axu\n");
					String line;
	 ...
	 * @return This object.
	 */
	public DjvSshShell start(long readTimeoutMs)
	{
		synchronized(this)
		{
