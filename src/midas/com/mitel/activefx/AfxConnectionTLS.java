...
public class AfxConnectionTLS extends AfxConnectionTcp
{
	...
	public AfxConnectionTLS(AfxDomain domain) throws SSLException
	{
		...
		{
			s_SslContext = MiSocketSslFactory.getInstance().getSslContext();
		}
	...
	public synchronized void initiateRead(FsmEvent evt)
	{
		...
		{
			MiSystem.logError(Category.DESIGN, "Invoked while already in reading mode at " + MiExceptionUtil.getCompressedTrace());
		}
	}
	...
	public void onRead()
	{
		synchronized(this)
		{
				...
				else
				{
					...
					while(--safety > 0)
					{
						...
						else if(res.getStatus() == SSLEngineResult.Status.BUFFER_OVERFLOW)
						{
							...
							MiSystem.logWarning(Category.DESIGN, "Data coming in faster than being processed");
							disableReactorRead();
							break;
						}
			...
			{
				MiSystem.logInfo(Category.DESIGN, "Closing channel due to " + e.getMessage());
				disableReactorRead();
				if(isHandShakingMode())
				{
					setHandshakeAborted(true);
	...
	public void onWrite()
	{
		synchronized(this)
		{
			...
			{
				MiSystem.logInfo(Category.DESIGN, "Channel closed due to " + e.getMessage());
	...
	private void openningHanshake()
	{
		synchronized(this) {
			...
			{
				MiSystem.logError(Category.DESIGN, "Already in handshaking mode");
			}
		}
	}
