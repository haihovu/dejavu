...
import com.mitel.mifsm.FsmException;
import com.mitel.miutil.MiWatchDog;
...
public class AfxDomain
{
	 ...
	 * @param maxChannels The maximum number of channels supported.
	 * This allow the domain to limit the amount of resource allocated.
	 * @param reactorThreadPriority The priority of the reactor thread. Valid values
	 * are integers in the range of Thread.MIN_PRIORITY(1)-Thread.MAX_PRIORITY(10). 
	 * Use values outside the valid range to denote default priority (same priority as calling thread).
	 * @throws IOException
	 */
	 ...
	 * @param wdPeriod Watchdog status reporting intervals, in milliseconds. Only has meaning if a watchdog is given.
	 * @param failureResponse Optional failure response logic, to be invoked if the watchdog detects a failure with this Active FX engine.
	 * @param reactorThreadPriority The priority of the reactor thread. Valid values
	 * are integers in the range of Thread.MIN_PRIORITY(1)-Thread.MAX_PRIORITY(10). 
	 * Use values outside the valid range to denote default priority (same priority as calling thread).
	 * @throws IOException
	 */
	public void start(int maxChannels, MiWatchDog watchdog, int wdPeriod, Runnable failureResponse, int reactorThreadPriority) throws IOException
	{
	...
	/**
	 * Dispatches an FSM event to the Active FX FSM domain.
	 * Intended to be used internally by other Active FX components.
	 * @param event The event to dispatch.
	 * @param queued Whether the event to be queued up to be processed by the FSM domain, or executed immediately by the calling thread.
	 * @return True if the event had either been queued up, or already processed (queued = false). 
	 * False if the event cannot be dispatched for any reason.
	 */
	...
	/**
	 * Used internally by the Active FX framework to retrieve the initial state for each Active FX FSM context.
	...
	/**
	 * Used internally by the Active FX framework to retrieve the FSM domain for this particular Active FX engine.
	...
	/**
	 * Retrieves the set of interested operations (from the reactor) for a specific handler
	 * @param handler The target handler
	 * @return The interest operation associated with the given handler. Basically a bit map of
	...
	int getInterestOps(ReactorEventHandler handler) {
		...
		if(reactor != null)
		{
			...
			MiSystem.logError(MiLogMsg.Category.DESIGN, "No reactor");
		}
		return 0;
	}
	
	/**
	 * Retrieves the set of ready operations (from the reactor) for a specific handler
	 * @param handler The target handler
	...
	/**
	 * Registers a specific handler for some set of operations, against the reactor.
	 * @param handler The target handler
	...
	/**
	 * De-registers some set of operations from a specific handler, against the reactor.
	 * @param handler The target handler
	 * @param events The set of operations of interest, basically a bit map of
	 * the different OP values in the class SelectionKey.
	 */
