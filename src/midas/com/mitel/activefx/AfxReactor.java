...
class AfxReactor implements Runnable
{
	...
	public synchronized void registerHandler(ReactorEventHandler handler, int events)
	{
				...
				if(interestOps != 0)
				{
					...
					else
					{
						MiSystem.logInfo(Category.DESIGN,
							"Already registered for " + events + " @ " + MiExceptionUtil.getCompressedTrace());
					}
				}
				else
				{
		...
		{
			MiSystem.logError(Category.DESIGN,
				"Channel " + handler.getHandle() + " encountered cancelled key @ " + MiExceptionUtil.simpleTrace(e));
		}
	}
	...
	public synchronized void deregisterHandler(ReactorEventHandler handler, int events)
	{
		if(null != handler.getHandle())
		{
			...
			if((null != key)&&(key.isValid()))
			{
				...
				MiSystem.logError(Category.DESIGN, "Invalid key");
		}
		else
		{
			MiSystem.logError(Category.DESIGN, "Null handle");
		}
	}
	...
	public void run()
	{
		...
		try
		{
			...
			MiSystem.logWarning(Category.DESIGN, this + " terminated due to " 
				+ MiExceptionUtil.simpleTrace(e));
		} catch(RuntimeException e) {
			MiSystem.logError(Category.DESIGN, this + " terminated due to " 
				+ MiExceptionUtil.simpleTrace(e));
		} finally {
			try {
				m_Selector.close();
