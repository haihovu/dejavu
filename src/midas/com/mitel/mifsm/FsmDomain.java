...
public abstract class FsmDomain implements Runnable
{
	 ...
	 */
	public synchronized void start(MiWatchDog watchdog, int wdPeriod, Runnable failureResponse)
	{
		if(!m_Started)
		{
			if(null != watchdog)
				...
				m_WdKey = m_Watchdog.registerComponent(getDomainName(), wdPeriod, new Runnable()
				{
					...
					public void run()
					{
						...
						if(null != dispatchThread)
						{
							...
							MiSystem.logWarning(Category.DESIGN, traceMsg.toString());
						}
	...
	public void stop()
	{
		MiSystem.logInfo(Category.DESIGN, getDomainName() + " stop requested by user");
		...
		try {
			if(queued)
			{
				synchronized(this)
				{
					if(m_DispatchingThread == null)
					{
						MiSystem.logWarning(Category.DESIGN, "FSM domain " + getDomainName() 
				...
				MiSystem.logWarning(Category.DESIGN, "Failed to send " + event);
			}
			else
			{
			...
			MiSystem.logError(Category.DESIGN, "Unexpected uninterrupted event");
		}
		return false;
	}
		...
		try
		{
			...
			catch(RuntimeException e)
			{
				MiSystem.logError(Category.DESIGN,
					Thread.currentThread() + " encountered " + MiExceptionUtil.simpleTrace(e));
			}
			catch(InterruptedException e)
			{
	...
	protected boolean handleEvent(FsmEvent event)
	{
		...
		else if(event == m_StatusEvent)
		{
			...
			if(wd != null)
			{
				if(!wd.reportStatus(key, MiWatchDog.Status.WD_NORMAL))
				{
					MiSystem.logWarning(Category.DESIGN,
	 ...
	 * @supplierCardinality 1 
	 */
	private final MiQueue m_EventQueue = new MiQueue(1000);
