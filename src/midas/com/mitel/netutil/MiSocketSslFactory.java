 ...
 * @author haiv
 */
public class MiSocketSslFactory {
	 ...
	 * This is a singleton
	 */
	private static MiSocketSslFactory m_Singleton;
	/**
	 ...
	 * @throws SSLException
	 */
	public MiSocketSslFactory(File storeFile, String storePass, String storeType, MiSslTrustListener listener) throws SSLException {
	 ...
	 * @throws SSLException
	 */
	public MiSocketSslFactory(File storeFile, String storePass, char[] keyPass, String storeType, MiSslTrustListener listener) throws SSLException {
		try {
			...
			if((keyPass != null) && (keyPass.length > 0)) {
				...
				keyMgrFactory.init(MiCertStore.loadStore(storeFile, storePass, storeType), keyPass);
	 ...
	 * @throws SSLException
	 */
	public MiSocketSslFactory(URL storePath, String storePass, String storeType, MiSslTrustListener listener) throws SSLException {
	 ...
	 * @throws SSLException
	 */
	public MiSocketSslFactory(URL storePath, String storePass, char[] keyPass, String storeType, MiSslTrustListener listener) throws SSLException {
		try {
			// Acquire the SSL context and claim it as ours
			m_SslContext = SSLContext.getInstance("SSL");
			if(null == m_SslContext) {
			...
			if((keyPass != null) && (keyPass.length > 0)) {
				KeyManagerFactory keyMgrFactory = KeyManagerFactory.getInstance("SunX509");
				keyMgrFactory.init(MiCertStore.loadStore(storePath, storePass, storeType), keyPass);
				keyMgrs = keyMgrFactory.getKeyManagers();
			}
	...
	public static void main(String[] args) {
		...
		for(Provider provider : Security.getProviders()) {
			MiSystem.logInfo(Category.DESIGN, "Found provider " + provider);
		}
		MiSystem.logInfo(Category.DESIGN, "Default trust manager algorith " + TrustManagerFactory.getDefaultAlgorithm());
		MiSystem.logInfo(Category.DESIGN, "Default key manager algorith " + KeyManagerFactory.getDefaultAlgorithm());
	}
	 ...
	 * @throws SSLException
	 */
	public static void loadTrustStore(File storeFile, String keyStorePass, String storeType, MiSslTrustListener listener) throws SSLException {
	 ...
	 * @throws SSLException
	 */
	public static void loadTrustStore(File storeFile, String keyStorePass, char[] keypass, String storeType, MiSslTrustListener listener) throws SSLException {
		synchronized(s_StaticLock) {
			if(m_Singleton == null) {
				m_Singleton = new MiSocketSslFactory(storeFile, keyStorePass, keypass, storeType, listener);
			}
		}
	}
	 ...
	 * @throws SSLException
	 */
	public static MiSocketSslFactory getInstance() throws SSLException {
		synchronized(s_StaticLock) {
			if(m_Singleton == null) {
				// Use default trust store
				m_Singleton = new MiSocketSslFactory(MiSocketSslFactory.class.getClassLoader().getResource("truststore"), "9x2F7ggYA81", null, "JCEKS", null/*no listener*/);
			}
			return m_Singleton;
		}
	}
	...
	private static class MiTrustManager implements X509TrustManager {
		 ...
		 * @throws SSLException
		 */
		private MiTrustManager(File storeFile, String storePass, String storeType, MiSslTrustListener listener)
			throws SSLException {
			m_StoreUrl = null;
			m_Listener = listener;
			...
			try {
				...
				m_KeyStore = MiCertStore.createStore(m_StoreFile, m_StorePass, storeType);
		 ...
		 * @throws SSLException
		 */
		private MiTrustManager(URL storePath, String storePass, String storeType, MiSslTrustListener listener)
			throws SSLException {
			storePath.getClass();
			m_StoreFile = null;
			m_Listener = listener;
			...
			try {
				...
				try {
					m_KeyStore = MiCertStore.createStore(iStream, m_StorePass, storeType);
				} finally {
					iStream.close();
				}
				...
				if(m_Listener != null) {
					if(m_Listener.challengeTrust(par1)) {
						try {
							...
							if(null != m_StoreFile) {
								MiCertStore.saveStore(m_KeyStore, m_StoreFile, m_StorePass);
							}
						}
						catch(Exception ex) {
		...
		private final MiSslTrustListener m_Listener;
		/**
