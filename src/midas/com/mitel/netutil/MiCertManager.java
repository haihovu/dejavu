 ...
 */
public class MiCertManager {
	private static MiCertManager defaultMgr;
	 ...
	 * @throws MiException 
	 */
	private MiCertManager(File keyStore, String storePass, String clientAlias, String clientPass) throws MiException {
		this.keyStore = MiCertStore.loadStore(keyStore, storePass, gSupportedStoreType);
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(cert.equals(c)) {
				MiSystem.logInfo(MiLogMsg.Category.DESIGN, "Success: " + cert + "->" + pem + "->" + c);
			} else {
				MiSystem.logWarning(MiLogMsg.Category.DESIGN, "Failed: " + cert + "->" + pem + "->" + c);
			}
		}
		catch(Exception ex) {
	...
	public static SSLSocketFactory getClientSocketFactory() throws SSLException {
		return MiSocketSslFactory.getInstance().getClientSocketFactory();
	}
	 ...
	 * @throws SSLException 
	 */
	public static MiCertManager initializeSslCertificate(File keystore, String storePass, String clientKeyAlias, String clientKeyPass) throws MiException, SSLException {
		// Initializes the SSL context with the given key/trust information
		MiSocketSslFactory.loadTrustStore(keystore, storePass, clientKeyPass.toCharArray(), gSupportedStoreType, new MiSslTrustListener() {
			@Override
			public boolean challengeTrust(X509Certificate[] par1) {
				return true;
			}
		});
		synchronized(gLock) {
			defaultMgr = new MiCertManager(keystore, storePass, clientKeyAlias, clientKeyPass);
			return defaultMgr;
		}
	}
	/**
	 ...
	 */
	public static MiCertManager getDefaultCertManager() {
		synchronized(gLock) {
			return defaultMgr;
		}
	}
