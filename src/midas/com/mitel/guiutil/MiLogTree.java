 ...
 * @author  haiv
 */
public class MiLogTree extends javax.swing.JTree
{
	 ...
	 */
	private final class BgTaskLogTreeUpdater extends MiBackgroundTask
	{
		/**
		...
		public void run()
		{
			synchronized(m_LogBuffer)
			{
				try
				{
					...
					SwingUtilities.invokeLater(new LogTreeUpdater(MiLogTree.this, m_LogBuffer, m_MaxSize, autoScroll));
					m_LogBuffer.clear();
				}
	...
	private class GetTopMsg implements Runnable{
		private MiLogMsg retValue;
		private boolean done;
		GetTopMsg() {
			super();
		}
		MiLogMsg waitForIt(long waitMs){
			synchronized(this){
				if(!done){
					if(waitMs > 0){
						try {
	 ...
	 */
    public MiLogTree(String name, int maxSize)
	{
		m_MaxSize = maxSize;
	 ...
	 * scrolled to the bottom if the bottom was previously in view.
	 */
	public void addMessage(MiLogMsg logMsg, boolean autoScroll)
	{
	 ...
	 * scrolled to the bottom if the bottom was previously in view.
	 */
	public void addMessage(MiLogMsg logMsg, int updateThresholdMs, boolean autoScroll)
	{
		synchronized(m_LogBuffer)
		{
	 ...
	 * scrolled to the bottom if the bottom was previously in view.
	 */
	public void addMessages(Collection<MiLogMsg> msgs, boolean autoScroll){
		...
		SwingUtilities.invokeLater(new LogTreeUpdater(MiLogTree.this, msgs, m_MaxSize, autoScroll));
	}
	 ...
	 */
	private final List<MiLogMsg> m_LogBuffer = new LinkedList<>();
	private MiBackgroundTask m_UpdateThread;
}
