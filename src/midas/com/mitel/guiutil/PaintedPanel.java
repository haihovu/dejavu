	...
	public PaintedPanel(java.lang.String imageName, java.lang.String cornerIcon, boolean tiled)
	{
		...
		if(null != imgUrl)
		{
			try
			{
				...
				{
					MiSystem.logError(Category.DESIGN, "Failed to get image for " + imgUrl);
				}
			}
			catch(IOException ex)
			{
				MiSystem.logError(Category.DESIGN, "Encountered " + ex
					+ " while loading the image " + imageName);
			}
		}
		else
		{
			MiSystem.logError(Category.DESIGN, "Failed to get URL for " + imageName);
		}
		...
		if(null != cornerIcon)
		{
			...
			if(null != iconUrl)
			{
				try
				{
					...
					{
						MiSystem.logError(Category.DESIGN, "Failed to get image for " + iconUrl);
					}
				}
				catch(IOException ex)
				{
			...
			{
				MiSystem.logError(Category.DESIGN, "Failed to get URL for " + cornerIcon);
			}
		}
		m_tiled = tiled;
	}
