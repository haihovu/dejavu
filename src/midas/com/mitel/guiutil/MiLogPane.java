 ...
 * MiLogPane takes care of updating the log tree with data from the MiSystem log framework.
 * </p>
 * <p>
 * A more convenient widget, the MiLogPanel provides in addition to the MiLogPane a filtering panel.
 * </p>
 * @author haiv
 */
public class MiLogPane extends JScrollPane implements ClipboardOwner
{
	 ...
	 */
	public MiLogPane(String title, int updateThresholdMs, int maxSize)
	{
		super();
		m_UpdateThresholdMs = updateThresholdMs;
		m_LogTree = new MiLogTree(title, maxSize);
						...
						if(callback  != null) {
							if(!atTop) {
								...
								if(top instanceof DefaultMutableTreeNode) {
									...
									if(userObj instanceof MiLogMsg) {
										index = ((MiLogMsg)userObj).m_Index;
									}
								}
	 ...
	 * @return This object
	 */
	public MiLogPane setListener(LogPaneListener listener) {
		synchronized(this){
	 ...
	 * @return This object
	 */
	public MiLogPane setAutoScroll(boolean value) {
		synchronized(this) {
			autoScroll = value;
		}
		return this;
	 ...
	 */
	public MiLogMsg addMessage(MiLogMsg msg){
		synchronized(this){
			...
			if((msg.getCategory() == MiLogMsg.Category.MAINTENANCE) && (!m_MaintenanceLogFilter)) {
				return msg;
			}

			if((msg.getCategory() == MiLogMsg.Category.DESIGN) && (!m_DesignLogFilter)) {
				return msg;
			}
		}
	 ...
	 */
	public void addMessages(Collection<MiLogMsg> msgs){
		// Validate the msgs
		long idx = -1;
		for(MiLogMsg msg : msgs) {
			if(idx < 0) {
				idx = msg.m_Index;
			} else {
				if(msg.m_Index > idx) {
		...
		List<MiLogMsg> filtered = new LinkedList<>();
		synchronized(this){
			int addMode = -1;
	 ...
	 * @return This object.
	 */
	public MiLogPane refreshLogPane() {
		synchronized(this) {
			refreshLogTree();
		}
		return this;
	 ...
	 * @return This object.
	 */
	public MiLogPane setDesignLog(boolean value)
	{
		synchronized(this)
		{
	 ...
	 * @return This object.
	 */
	public MiLogPane setMaintenanceLog(boolean value)
	{
		synchronized(this)
		{
	 ...
	 * @return This object.
	 */
	public MiLogPane setSeverityLevel(int level){
		synchronized(this){
	...
	private void refreshLogTree()
	{
		...
		List<MiLogMsg> logs = new LinkedList<>();
		synchronized(MiLogPane.this) {
			int severity = getSeverityLevel();
			for(MiLogMsg msg : m_LogBufferCache){
				// ... don't forget to apply any filter
				if((!m_MaintenanceLogFilter)&&(msg.getCategory() == MiLogMsg.Category.MAINTENANCE)){
					continue;
				}
				if((!m_DesignLogFilter)&&(msg.getCategory() == MiLogMsg.Category.DESIGN)) {
					continue;
				}
	...
	private final MiLogTree m_LogTree;
	 ...
	 */
	private final LinkedList<MiLogMsg> m_LogBufferCache = new LinkedList<>();
