...
public final class LogTreeUpdater implements Runnable
{
	...
	private final List<MiLogMsg> m_NewLogBuffer;
	 ...
	 */
	public LogTreeUpdater(JTree tree, Collection<MiLogMsg> logBuffer, int maxTreeSize, boolean autoScroll)
	{
		super();
		m_Tree = tree;
	...
	public void run()
	{
		...
		boolean scrolling = MiGuiUtil.autoScrollIsDesirable(m_Tree) && autoScroll;
		/*
		...
		*/
		...
		for(MiLogMsg msg : m_NewLogBuffer){
				...
				while(true) {
					...
					if(m_TreeRoot.getChildCount() > 0) {
						...
						if(bottom instanceof DefaultMutableTreeNode) {
							...
							if(obj instanceof MiLogMsg) {
								MiLogMsg bottomMsg = (MiLogMsg)obj;
						...
						if(top instanceof DefaultMutableTreeNode) {
							...
							if(obj instanceof MiLogMsg) {
								if(msg.m_Index < ((MiLogMsg)obj).m_Index) {
									// New msg is smaller than top, insert at top
									mode = InsertionMode.TOP;
									topLogIndex = ((MiLogMsg)obj).m_Index;
									break;
								}
							} else {
