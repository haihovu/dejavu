...
public class AfxConnectionUdp extends AfxConnection implements ReactorEventHandler, FsmContext {
		...
		try {
			...
			if (null != m_RemoteAddress) {
				...
				if (0 < (byteWritten = channel.send(m_WriteBuffer, m_RemoteAddress))) {
					...
					DjvSystem.logWarning(Category.DESIGN, "Send failed");
				}
			} else {
				DjvSystem.logWarning(Category.DESIGN, "No remote address");
			}
	...
	public String toString() {
		if (null != channel) {
			return "(AfxConnection " + channel.socket().getLocalSocketAddress() + ")";
		}
	...
	@Override
	@SuppressWarnings("NestedAssignment")
	void initiateOpen(FsmEvent evt) throws InterruptedException {
	...
	@Override
	void initiateClose(FsmEvent evt) throws InterruptedException {
		if (channel != null) {
			try {
				channel.close();
			} catch (IOException e) {
	...
	@Override
	void initiateConnect(FsmEvent evt) throws InterruptedException {
		AfxFsmEvent afxEvent = (AfxFsmEvent) evt;
		try {
			connectionventHandler = afxEvent.getEventHandler();
			...
			if(afxEvent.channel instanceof DatagramChannel) {
				channel = (DatagramChannel)afxEvent.channel;
				channel.configureBlocking(false);
			} else {
				channel = DatagramChannel.open();
