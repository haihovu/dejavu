...
/**
 * The central concept of the Reactor pattern. These are active objects that
 * detects read/write/connect events on a number of registered channels. Events
 * are dispatched to the registered event handlers for processing.
 */
class AfxReactor implements Runnable {
	 ...
	 *
	 * @param name The name of the reactor to be created.
	 * @param maxChannels The maximum number of channels that can be supported
	 * by this reactor.
	 * @throws IOException
	 */
	 ...
	 * @return The interested operation associated with the given handler.
	...
	public synchronized int getInterestOpsFor(ReactorEventHandler handler) {
		...
		SelectionKey key = channel != null ? channel.keyFor(selector) : null;
		if (key != null) {
			if (key.isValid()) {
	 ...
	 * @param events The set of interested events (bit-masked of SelectionKey OP
	 * flags).
	 */
	public synchronized void registerHandler(ReactorEventHandler handler, int events) {
		try {
			...
			SelectionKey key = handler.getHandle().keyFor(selector);
			if ((null == key) || (!key.isValid())) {
				if (0 != events) {
					...
					key = handler.getHandle().register(selector, events);
					if (null != key) {
						key.attach(handler);
					} else {
	 ...
	 * If the specified handler is still registered against some other events it
	 * will continue to be served by the reactor.
	 *
	...
	/**
	 * Remove an event handler from the reactor no matter if it is still
	 * registered with some set of events. This is equivalent to deregistering
	 * the handler against all events.
	 *
	...
	public void run() {
		...
		int[] interestArray = new int[maxChannels];
		ReactorEventHandler[] handlerArray = new ReactorEventHandler[maxChannels];
		SelectableChannel[] channelArray = new SelectableChannel[maxChannels];
						...
						for (SelectionKey key : selectedKeys) {
							if (key.isValid()) {
								if (keyCount < maxChannels) {
									...
									DjvSystem.logError(Category.DESIGN,
											name + ": number of channels, " + selectedKeys.size() + " exceeded maximum of " + maxChannels);
								}
							}
						}
						selectedKeys.clear();
	 ...
	 *
	 * @param threadPriority The priority of the reactor thread. An integer in
	 * the range of Thread.MIN_PRIORITY(1)-Thread.MAX_PRIORITY(10). Use zero or
	 * other values outside the valid range to denote default priority (same
	 * priority as calling thread).
	 */
	public synchronized void start(int threadPriority) {
		if (!started) {
			started = true;
			Thread myThread = new Thread(this, name + "Reactor");
	...
	/**
	 * Stops the reactor, destroying its active thread. Once stopped the reactor
	 * should be discarded and not be reused.
	 */
	public void stop() {
		try {
			running = false;
	...
	public String toString() {
		synchronized (this) {
			return "AfxReactor: {" + "selector:" + selector + ", started:" + started + ", running:" + running + ", name:" + name + ", maxChannels:" + maxChannels + '}';
		}
	}
