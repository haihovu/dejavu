...
import org.dejavu.util.DjvLogHandler;

/**
 * A log level panel with filtering user interface. This wraps around the DjvLogPane
 widget and adds a filter panel.
 * @author haiv
 */
public class DjvLogPanel extends javax.swing.JPanel
{
	 ...
	 */
	private class BgTaskNumMsgsUpdater extends DjvBackgroundTask
	{
		...
		public void run()
		{
			synchronized(DjvLogPanel.this)
			{
				try
				{
					if(lWaitMs > 0)
					{
						DjvLogPanel.this.wait(lWaitMs);
					}
	 ...
	 */
	private class LocalLogHandler implements DjvLogHandler{
		/**
		...
		public DjvLogMsg logMsg(DjvLogMsg.Category category, int severity, Class origClass, String origMethod, String message){
			DjvLogMsg msg = new DjvLogMsg(category, severity, origClass, origMethod, message);
		 ...
		 */
		private void logMsgs(Collection<DjvLogMsg> msgs) {
	 ...
	 */
    public DjvLogPanel(String title, int updateThresholdMs, int maxLogMsgs, boolean designLog, boolean maintLog, int severity)
	{
		m_Title = title;
		m_UpdateThresholdMs = updateThresholdMs;
		m_LogPane = new DjvLogPane(title, updateThresholdMs, maxLogMsgs).setDesignLog(designLog).setMaintenanceLog(maintLog).setSeverityLevel(severity);
	 ...
	 * @return This object
	 */
	public DjvLogPanel registerListener(DjvLogPane.LogPaneListener listener){
	...
	/**
	 * Retrieves the log handler. This is typically used to feed into DjvSystem.registerLogHandler().
	 * @return The log handler.
	 */
	public DjvLogHandler getLogHandler()
	{
		return m_LogHandler;
	}
	 ...
	 * @return This object.
	 */
	public DjvLogPanel addMessage(DjvLogMsg msg) {
	 ...
	 * @return This object.
	 */
	public DjvLogPanel addMessages(Collection<DjvLogMsg> msgs) {
	 ...
	 * @return This object
	 */
	public DjvLogPanel clearMessages() {
	 ...
	 * @return This object.
	 */
	public DjvLogPanel refreshLogPane() {
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				final LinkedList<DjvLogMsg> logs = new LinkedList<>();
				final JFrame frame = new JFrame("Test");
				final DjvLogPanel logpanel = new DjvLogPanel("Logs", 1000, 1024, true, true, 2);
				logpanel.registerListener(new DjvLogPane.LogPaneListener() {
					@Override
					public void scrolledToTop(final long topIndex) {
						new Thread(new Runnable() {
							@Override
							public void run() {
								DjvSystem.logInfo(DjvLogMsg.Category.DESIGN, "Scrolled to the top mate: " + topIndex);
								if(topIndex > 0) {
									try {
										...
										Collection<DjvLogMsg> olderLogs = new LinkedList<>();
										synchronized(logs) {
					...
					public void scrolledToBottom(final long bottomIndex) {
						new Thread(new Runnable() {
							@Override
							public void run() {
								DjvSystem.logInfo(DjvLogMsg.Category.DESIGN, "Scrolled to the bottom mate: " + bottomIndex);
								try {
									synchronized(logs) {
										int logSize = logs.size();
										if(bottomIndex < (logSize - 1)) {
											Collection<DjvLogMsg> newerLogs = new LinkedList<>();
											int count = 0;
					...
					public void scrolledOffTop() {
						DjvSystem.logInfo(DjvLogMsg.Category.DESIGN, "Scrolled off the top mate");
					}

					@Override
					public void scrolledOffBottom() {
						DjvSystem.logInfo(DjvLogMsg.Category.DESIGN, "Scrolled off the bottom mate");
					}
				});
				...
				JButton logGen = new JButton(new AbstractAction("Log gen") {
					...
					public void actionPerformed(ActionEvent ae) {
						synchronized(logs){
							for(int i = 0; i < 100; ++i){
								DjvLogMsg log = new DjvLogMsg(DjvLogMsg.Category.DESIGN, 2, DjvLogPanel.class, "main", "New log " + System.currentTimeMillis());
								logs.add(log);
				...
				synchronized(logs){
					...
					for(int i = 0; i < logSize; ++i) {
						DjvLogMsg.Category cat = (i % 2 == 0) ? DjvLogMsg.Category.DESIGN : DjvLogMsg.Category.MAINTENANCE;
						DjvLogMsg log;
						switch(i % 3) {
							case 0:
								log = new DjvLogMsg(cat, 0, DjvLogPanel.class, "main", "Test log # " + i);
								break;

							case 1:
								log = new DjvLogMsg(cat, 1, DjvLogPanel.class, "main", "Test log # " + i);
								break;

							default:
								log = new DjvLogMsg(cat, 2, DjvLogPanel.class, "main", "Test log # " + i);
								break;
						}
						logs.add(log);
						logpanel.addMessage(log);
