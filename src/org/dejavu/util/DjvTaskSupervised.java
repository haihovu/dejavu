...
/**
 * Like a background task but can be monitored and restarted (by a task manager)
 * if premature termination is detected. Concrete implementation should override
 * (implement) the abstract method mainLoop (as opposed to the method run). When
 * started a worker thread is spawned, which then executes the mainLoop method.
 *
 * @author haiv
 */
public abstract class DjvTaskSupervised implements Runnable {
	...
	/**
	 * Listener interface for receiving asynchronous progress/status
	 * notifications regarding supervised tasks.
	 */
	...
	private final Set<Listener> listeners = new HashSet<>(16);
	 ...
	 *
	 * @param taskManager An optional task manager which perform supervision
	 * functionality. If specified this task will automatically register with
	 * the task manager once the worker thread is up and running, and will
	 * deregister when the worker thread terminates.
	 ...
	 * Stops the task.
	 *
	 * @param timeoutMs Timeout to wait for the task to terminate. Positive
	 * integers represents the number of milliseconds to wait, zero means wait
	 * forever, negative values means no wait.
	 * @throws InterruptedException
	 */
	 ...
	 *
	 * @return The current worker thread for this task, or null if the task had
	 * been stopped or had terminated somehow.
	 */
	...
	/**
	 * Retrieves the run flag for the task, this indicates that the task was
	 * started. This flag is set to true by the start method, and set to false
	 * by the stop method. Main loops should monitor this flag and terminate
	 * when set to false. Note that this doesn't mean the worker thread is
	 * currently running.
	 *
	...
	/**
	 * This is the main loop for the supervised task. Concrete supervised task
	 * implementation should override this method and provide their custom
	 * logic.
	 */
