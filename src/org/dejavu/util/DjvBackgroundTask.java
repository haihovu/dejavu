...
public abstract class DjvBackgroundTask implements Runnable {
	...
	/**
	 * Starts up the background task, i.e. starts the worker thread. Should be
	 * invoked only once, though I don't think repeated invocation hurts.
	 *
	 * @return This object.
	 */
	...
	/**
	 * Determines whether the background task is still running, i.e. the worker
	 * thread is alive.
	 *
	 * @return True if the background task is still running, false if it had
	 * completed or stopped somehow.
	 */
	...
	/**
	 * Retrieves the run flag, which is set to true when the start method is
	 * invoked, and set to false when the stop method is invoked.
	 *
